...
`gosh_ackinacki`). The public, MIT-licensed mirror lives at
**[`gosh-dot-ai/gosh.ackinacki`](https://github.com/gosh-dot-ai/gosh.ackinacki)** —
depend on it with the **lean default** (no node, no MsQuic): a thin chain client
over the Block Manager HTTP/GraphQL API.
...
### The `sdk` facade (recommended)

`gosh_ackinacki::sdk` is a small, typed surface over the lean (Plane A) layers —
no MCP server, no indexer. Runnable demo:
[`examples/sdk_lean_client.rs`](examples/sdk_lean_client.rs)
(`cargo run --example sdk_lean_client`).

| Type | What |
|------|------|
| `ChainClient` | `connect` / `shellnet` · `get_account` (+ native & ECC balances) · `run_getter` · `call` (signed write) · `deploy` (poll-to-Active) · `chain_liveness` · `subscribe_events` (a `futures::Stream` over the event log) |
| `ChainClient` (inference market) | `inference_order_book` — a consistent snapshot of **all open orders** (bids/asks, best bid/ask, stats) of a per-model order book · `subscribe_inference_events` — a `Stream` of typed market events (placed / cancelled / **filled** / executed / …) |
| `InferenceEvent` / `OrderBookSnapshot` / `OpenOrder` | typed, serde-serializable inference-market views (see `gosh_ackinacki::inference`; also decodable from block-stream `MatchedMessage` bodies) |
| `Wallet` | a `SwarmMultisigWallet` handle: `submit` / `submit_full` (value + ECC + payload) / `confirm` |
| `KeyPair` | the **ed25519** signing identity: `generate` / `from_secret_hex` / `sign(&[u8])` / `verify` — zeroizing secret, not `Clone` |
| `BoxKey` | **authenticated X25519 sealing** (NaCl `crypto_box`): `encrypt_to(recipient_pub, msg)` / `open(sender_pub, &sealed)` — seal a credential/endpoint to a peer's published key |
| `Address` / `Pubkey` / `Signature` | validated, canonical hex newtypes |

For the **block-stream indexer** (Plane B), the market is a preset:
`AbiRegistry::with_inference_market()` (decodes order-book / note events from
ANY address via global ABIs) + `FilterRule::inference_market_events()` /
`FilterRule::private_note_events()` — then `process_block` yields the matched
messages and `inference::events::decode_event_body` types them.

```rust
use gosh_ackinacki::sdk::{ChainClient, Address, KeyPair, BoxKey};

let client = ChainClient::shellnet()?;
let live = client.chain_liveness().await?;                       // is the chain producing blocks?
let acc  = client.get_account(&Address::parse("0:<hex>")?).await?;  // status + native/ECC balances
// client.call(&addr, abi_json, "method", args, &keys).await?;   // signed external-inbound write

let keys = KeyPair::generate();                                  // ed25519 signing identity
let sig  = keys.sign(challenge)?;                                // sign an arbitrary payload

let seller = BoxKey::generate()?;                                // X25519 box identity
let sealed = seller.encrypt_to(buyer_box_pub_hex, endpoint.as_bytes())?;  // authenticated seal
```

> **Two key types, on purpose.** `KeyPair` (ed25519) signs blockchain messages and
> arbitrary payloads. `BoxKey` (X25519, NaCl box) seals/opens messages to/from a
> peer — a *separate* identity for a different job. Both are built on the Acki
> Nacki / TVM crypto stack (`tvm_client::crypto`, tvm-sdk `v3.0.3-rc.an`); **neither
> depends on gosh.memory**.

The facade wraps these lower-level modules, available directly for advanced use:
`airegistry::getter` (`AccountReader`, `read_events`), `airegistry::run::GetterRunner`,
`airegistry::deploy` / `airegistry::calls`, `wallet::query::send_message`,
`wallet::transact`, `config::NetworkConfig`.

### Build modes

- **lean (default)** — Plane A only (HTTP/GraphQL). Self-contained from **public**
  repos (`gosh-ackinacki` + `tvmlabs/tvm-sdk`); no node, no MsQuic.
- **`--features block-stream`** — the full-agent indexer (Plane B): a QUIC firehose
  from Block Keeper nodes (`transport` → `decoder` → `filter::engine` → `transform`).
  Pulls the **public** node [`ackinacki/ackinacki`](https://github.com/ackinacki/ackinacki)
  + MsQuic (needs `cmake`/`clang`, see Prerequisites). Off by default — lean
  consumers never compile it.
- **`--features private-note`** — in-process DEX **PrivateNote** minting
  (`private_note::deploy_private_note_from_multisig`): the wallet-funded
  `RootPN.generateVoucher` → `deployPrivateNote` → `sendEccShellToPrivateNote`
  path with a halo2 voucher prover. Pulls the **public** halo2 proving graph
  (`gosh-sh/dexdo-halo2-kit`, `halo2-lib-…`) — heavy; off by default.
