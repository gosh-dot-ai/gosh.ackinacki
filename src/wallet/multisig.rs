...
/// `UpdateCustodianMultisigWallet_v2` (v2.2.0) — the canonical dexdo funding
/// wallet, from `gosh-sh/ackinacki-kit` v5.0.0. 7-arg `sendTransaction` (trailing
/// `dapp_id`). Deployable. See `contracts/msig/PROVENANCE.md`.
...
pub enum MultisigKind {
    ...
    UpdateCustodianV1,
    /// `UpdateCustodianMultisigWallet_v2` — canonical dexdo funding wallet.
    UpdateCustodianV2,
}
...
impl MultisigKind {
        ...
        match h.as_str() {
            ...
            other => Err(anyhow!(
                "unsupported multisig code_hash {other}; supported: generic Multisig \
                 ...
                 {UPDATE_CUSTODIAN_V1_CODE_HASH}, v2 {UPDATE_CUSTODIAN_V2_CODE_HASH}"
            )),
        }
    }
    ...
    /// v1 is 6-arg; the generic Multisig and v2 are 7-arg.
    pub fn has_dapp_id_arg(self) -> bool {
        matches!(self, Self::Generic | Self::UpdateCustodianV2)
    }
...
pub async fn deploy_multisig(
    ...
    let ctor = json!({
        ...
        "owners_address": [],
        "reqConfirms": params.req_confirms,
        "reqConfirmsData": params.req_confirms,
        "value": "0",
    });
