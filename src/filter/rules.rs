...
/// Event names emitted by the `InferenceOrderBook` contract — the market feed
/// (orders placed / cancelled / filled, executions, refunds, subscriptions).
pub const INFERENCE_ORDER_BOOK_EVENTS: &[&str] = &[
...
/// Event names emitted by a trader's `PrivateNote` (order confirmations,
/// private transfers) and by `RootPN` (note lifecycle).
pub const PRIVATE_NOTE_EVENTS: &[&str] = &[
    "InferenceOrderPlacedConfirmed",
    "InferenceFilledConfirmed",
    "OrderSubmitted",
...
impl FilterRule {
    /// Everything the inference market emits: order-book events + note-side
    /// confirmations, from ANY address (books/notes are deployed dynamically).
    /// Requires the matching ABIs in the engine's `AbiRegistry` (see
    /// `AbiRegistry::with_inference_market`) so ext-out bodies decode to names.
    pub fn inference_market_events() -> Self {
        Self {
            msg_types: HashSet::from([MsgTypeFilter::ExtOut]),
            method_names: INFERENCE_ORDER_BOOK_EVENTS
                .iter()
                .chain(PRIVATE_NOTE_EVENTS)
                .map(|s| s.to_string())
                .collect(),
            ..Default::default()
        }
    ...
    /// Only PrivateNote / RootPN (note) events — trader activity and note
    /// lifecycle, without the order-book feed.
    pub fn private_note_events() -> Self {
        Self {
            msg_types: HashSet::from([MsgTypeFilter::ExtOut]),
            method_names: PRIVATE_NOTE_EVENTS.iter().map(|s| s.to_string()).collect(),
            ..Default::default()
        }
    }
...
mod tests {
    ...
    #[test]
    fn inference_market_preset_matches_book_and_note_events_any_address() {
        let rule = FilterRule::inference_market_events();
        // Order-book fill from an address we never configured.
        assert!(rule.matches(
            Some("0:dynamic-book"),
            None,
            &MsgTypeFilter::ExtOut,
            Some("InferenceFilled"),
        ));
        // Note-side confirmation.
        assert!(rule.matches(
            Some("0:some-note"),
            None,
            &MsgTypeFilter::ExtOut,
            Some("InferenceOrderPlacedConfirmed"),
        ));
        // Non-market event name → no match.
        assert!(!rule.matches(
            Some("0:dynamic-book"),
            None,
            &MsgTypeFilter::ExtOut,
        ...
        // Right name but not an ext-out (an ext-in CALL, not an event) → no match.
        assert!(!rule.matches(
            None,
            Some("0:book"),
            &MsgTypeFilter::ExtIn,
    ...
    #[test]
    fn private_note_preset_excludes_book_feed() {
        let rule = FilterRule::private_note_events();
        assert!(rule.matches(
            Some("0:note"),
            None,
            &MsgTypeFilter::ExtOut,
            Some("TransferReceived"),
        ));
        assert!(rule.matches(
            Some("0:rootpn"),
            None,
            &MsgTypeFilter::ExtOut,
            Some("VoucherGenerated"),
        ));
        assert!(!rule.matches(
            Some("0:book"),
            None,
