...
use crate::private_note::halo2::cache::ProverCacheStorage;
...
/// but uses `anyhow::Result` for ergonomics.
#[derive(Debug, Clone)]
...
pub async fn export_witness(params: &ExportWitnessParams) -> Result<String> {
    witness_lib::make_private_witness_and_public_data(&params.into())
        .await
        .map_err(|e| anyhow!("dark-DEX witness export failed: {e}"))
}
...
pub fn generate_proof(cache: &impl ProverCacheStorage, fixture_json: &str) -> Result<ProofOutput> {
    let cache_dir = cache.pk_dir();
    let cached = cache_dir
        .as_ref()
        .map(|d| d.join("pk_cache.bin").exists())
        .unwrap_or(false);

    let t_init = std::time::Instant::now();
    // Prover initialiser used by the live-proven gosh-ackinacki-v0.4.1 baseline
    // with kit rev 89e9346 (whose `gosh.zkhalo2verify` accepts the produced
    // circuit). NOTE: with this kit rev a `None` URL resolves to the kit's default
    // Hermez KZG SRS and will fetch it over HTTP into the prover cache dir if
    // `hermez_kzg_srs_k19.bin` is absent (it is the SRS this prover consumes — NOT
    // the legacy `kzg_bn254` file `Halo2Paths::ensure_srs` still provisions). SRS
    // provisioning/caching and running this CPU-bound prover off the async thread
    // are SDK/prover concerns tracked by dexdo #476 (open) and are NOT changed
    // here; this PR only ensures the pinned rev's circuit is used.
    ...
    let out = prover
        .generate_proof(fixture_json)
        .map_err(|e| anyhow!("halo2 proof generation failed: {e}"))?;
    eprintln!(
...
///
/// Mirrors the acki-nacki Python pipeline's one-window safety margin: the
/// witness lib's internal readiness check compares the rounded-up bucket
/// boundary with the latest block, but the dense-chain leaf for the boundary
/// bucket is only materialised on the chain a few blocks past the boundary
/// itself.
/// On events landing exactly on a boundary (e.g. bh % W == 0), waiting
/// just for `latest >= boundary` makes witness export fail with
/// `block at height boundary+k not found`. The extra window gives the node
/// time to seal the leaf-bearing block.
