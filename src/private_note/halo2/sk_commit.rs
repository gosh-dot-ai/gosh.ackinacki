...
pub fn compute_sk_u_commit(sk_u_hex: &str) -> Result<[u8; 32]> {
    let stripped = sk_u_hex
        .strip_prefix("0x")
        .or_else(|| sk_u_hex.strip_prefix("0X"))
        .unwrap_or(sk_u_hex);

    let bytes_vec = hex::decode(stripped).map_err(|e| anyhow!("sk_u hex decode failed: {e}"))?;

    let sk_u_bytes: [u8; 32] = bytes_vec
        .try_into()
        .map_err(|v: Vec<u8>| anyhow!("sk_u must decode to exactly 32 bytes, got {}", v.len()))?;

    let sk_u: Fr = Option::from(Fr::from_repr(sk_u_bytes))
        .ok_or_else(|| anyhow!("sk_u is not a valid BN254 field element"))?;
...
pub fn compute_sk_u_commit_hex(sk_u_hex: &str) -> Result<String> {
