...
impl Halo2Paths {
    ...
    pub fn validate(&self) -> Result<(), Halo2PathsError> {
        ...
        ensure_writable_dir(&self.fixture_dir).map_err(|source| {
            Halo2PathsError::FixtureDirNotWritable {
    ...
    ///
    /// Mutating the process env is not thread-safe. Call once at boot from a
    /// single thread, before any halo2 work starts.
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::ProverCacheDirNotWritable { path, source } => {
                write!(
                    f,
                    "prover cache dir {} not writable: {source}",
                    path.display()
                )
            }
