...
use crate::private_note::halo2::cache::FilesystemCache;
use crate::private_note::halo2::proof as halo2_proof;
use crate::private_note::halo2::proof::ExportWitnessParams;
...
pub struct ProveVoucherForEventParams<'a> {
    pub endpoint: String,
    pub event: voucher_event::VoucherExtoutMessage,
    pub sk_u_hex: String,
    pub sk_u_commit_hex: String,
    pub voucher_value: u64,
    pub voucher_token_type: u32,
    pub ephemeral_pubkey_hex: String,
    pub history_proof_window_size: Option<u64>,
    pub paths: &'a super::paths::Halo2Paths,
}
...
///
/// Returns `Err` on any halo2 / chain failure (misconfigured `paths` —
/// missing SRS, unwritable cache/fixture dir; layer wait timeout; witness
/// export failure; prover failure across the whole attempt plan), so an
/// in-process host embedding this library never crashes on a transient
/// network or proving error.
pub async fn prove_voucher_for_event(params: ProveVoucherForEventParams<'_>) -> Result<Halo2Proof> {
    let ProveVoucherForEventParams {
        endpoint,
        event,
        sk_u_hex,
        sk_u_commit_hex,
    ...
    let block_id = event
        .block_id
        .as_deref()
        .ok_or_else(|| anyhow!("event block_id not resolved before prove"))?;
    let block_height = voucher_event::get_block_height_by_id(&http, &endpoint, block_id)
        .await
        .context("block height query")?
        .ok_or_else(|| anyhow!("no block found for event block_id {block_id}"))?;
    eprintln!(
    ...
    for &layer in &plan {
        ...
        let latest = voucher_event::get_latest_block_height(&http, &endpoint)
            .await
            .ok();
    ...
    bail!("prove_voucher_for_event: halo2 proof failed for all attempted layers {plan:?}");
}
...
fn into_halo2_proof(
    ...
    Ok(Halo2Proof {
        ...
        voucher_value,
        voucher_token_type,
        layer_number: u8::try_from(layer.saturating_add(1))
            .map_err(|_| anyhow!("layer {layer} does not fit into u8"))?,
        sk_u_hex,
        sk_u_commit_hex,
    })
}

fn fixture_path_for(dir: &std::path::Path, block_height: u64, layer: u32) -> Result<PathBuf> {
    ...
    Ok(dir.join(format!(
        "dex_fixture_live_L{layer}_H{block_height}_T{nanos}.json"
    )))
}
