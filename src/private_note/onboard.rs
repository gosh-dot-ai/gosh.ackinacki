...
/// PrivateNote minting MUST be serial. The halo2 prover writes a shared on-disk
/// cache (`pk_cache.bin` / `break_points_cache.bin` / `vk_cache.bin`, at fixed
/// names under `Halo2Paths::prover_cache_dir`) and the funding wallet mints one
/// voucher at a time, so two deploys running at once would corrupt the cache and
/// race the wallet. This is a process-wide latch enforcing an EXPLICIT
/// prohibition — a concurrent deploy fails fast instead of silently racing.
static MINT_IN_PROGRESS: AtomicBool = AtomicBool::new(false);

/// RAII latch: taken for the whole deploy, released on drop (incl. early return /
/// error / panic), so a failed mint never leaves the latch stuck.
#[derive(Debug)]
struct MintGuard;
...
impl MintGuard {
    fn acquire() -> Result<Self> {
        ...
        {
            return Err(anyhow!(
                "a PrivateNote deploy is already in progress in this process — note minting must \
                 be SERIAL (the halo2 prover cache and the funding wallet cannot be shared by \
                 concurrent mints). Wait for the in-flight deploy to finish, then retry; do not \
...
impl Drop for MintGuard {
    fn drop(&mut self) {
        MINT_IN_PROGRESS.store(false, Ordering::SeqCst);
    }
}
...
pub async fn deploy_private_note_from_multisig(
    ...
    // Enforce serial minting up front: fail fast if another deploy is in flight,
    // before any wallet spend. Held for the whole deploy (released on drop).
    let _mint_guard = MintGuard::acquire()?;
...
mod tests {
    ...
    #[test]
    fn mint_guard_enforces_serial_minting() {
        // First acquire succeeds; a second while held fails fast with the serial
        // -minting message; after the first drops, acquire succeeds again.
        let g1 = MintGuard::acquire().expect("first acquire");
        let err = MintGuard::acquire().expect_err("second concurrent acquire must fail");
        assert!(
            err.to_string().contains("must be SERIAL"),
            "explicit serial-minting error: {err}"
        );
        drop(g1);
        let _g2 = MintGuard::acquire().expect("acquire after release");
    }
}
