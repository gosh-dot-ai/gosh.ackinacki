...
use crate::wallet::query::{send_message, send_message_routed};
...
impl ChainClient {
    ...
    /// The shared TVM SDK client context (for building deploy / call messages).
    ...
    /// Like [`ChainClient::call`] but routes the signed message to `address` in
    /// an **explicit** DApp, instead of assuming `dapp == address` (what
    /// [`ChainClient::call`] derives). Needed for accounts that live in a DApp
    /// other than their own id — e.g. a DEX **PrivateNote**, which is deployed
    /// inside a system/registry DApp (its id is a network-config value, not the
    /// note's address). A generic transport service: the caller supplies the
    /// ABI / method / args (its own contract logic) and the DApp the target
    /// lives in; this crate does not embed any particular DApp id.
    pub async fn call_in_dapp(
        &self,
        address: &Address,
        dapp: &Address,
        abi_json: &str,
    ...
    /// A consistent snapshot of an inference-market order book: ALL open orders
    /// (bids/asks in price-time order), best bid/ask, stats, model identity.
    /// One BOC fetch; every getter runs locally. `Ok(None)` if the book is not
    /// deployed/active. `max_scan` caps the order-id probe (e.g. `10_000`).
    ///
    /// `dapp`: the DApp the book lives in. `None` auto-resolves — its own id
    /// first, then the system DApp (note-deployed books inherit their deployer's
    /// DApp lineage, which starts at RootPN in the system DApp).
    pub async fn inference_order_book(
        &self,
        address: &Address,
        dapp: Option<&Address>,
        max_scan: u128,
    ) -> Result<Option<crate::inference::OrderBookSnapshot>> {
        let addr = address.with_workchain();
        match dapp {
            Some(d) => {
                let origin = AccountOrigin::Explicit {
                    dapp_id: d.bare().to_string(),
                };
                crate::inference::fetch_order_book(
                    &self.reader,
                    &self.runner,
                    &self.cfg,
                    &addr,
            ...
            None => {
                // Auto: own DApp first, then the system DApp.
                if let Some(book) = crate::inference::fetch_order_book(
                    &self.reader,
                    &self.runner,
                    &self.cfg,
                    &addr,
                ...
                let system = AccountOrigin::Explicit {
                    dapp_id: crate::inference::book::SYSTEM_DAPP_ID.to_string(),
                };
                crate::inference::fetch_order_book(
                    &self.reader,
                    &self.runner,
    ...
    /// Subscribe to an address's **inference-market events** as an async
    /// `Stream` of typed [`crate::inference::InferenceEvent`]s. Point it at an
    /// order book for the whole market feed of one model (placed / cancelled /
    /// filled / executed…), or at a PrivateNote for that trader's activity.
    /// `dapp` follows [`ChainClient::inference_order_book`] (pass the DApp the
    /// account lives in; `None` = the account's own id — note-deployed books
    /// usually need the system DApp instead). Same pagination/checkpoint
    /// semantics as [`ChainClient::subscribe_events`]; foreign messages are
    /// skipped.
    pub fn subscribe_inference_events(
        &self,
        address: &Address,
        dapp: Option<&Address>,
        page: u32,
        poll: Duration,
    ) -> impl Stream<Item = Result<EventRecord<crate::inference::InferenceEvent>>> + 'static {
        let reader = self.reader.clone();
        let cfg = self.cfg.clone();
        let addr = address.with_workchain();
        let origin = match dapp {
            Some(d) => AccountOrigin::Explicit {
                dapp_id: d.bare().to_string(),
            },
        ...
        futures_util::stream::unfold(
            (
                None::<String>,
                VecDeque::<EventRecord<crate::inference::InferenceEvent>>::new(),
            ),
            move |(mut cursor, mut buf)| {
                ...
                async move {
                    loop {
                        ...
                        let abis = match crate::inference::InferenceAbis::load() {
                            Ok(a) => a,
                            Err(e) => {
                        ...
                        match reader
                            .read_events_with(
                                ...
                                cursor.as_deref(),
                                &|body_b64| {
                                    crate::inference::decode_event_body_b64(&abis, body_b64)
                                        .ok()
                                        .flatten()
                                },
                            )
