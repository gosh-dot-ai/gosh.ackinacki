...
/// The **current** system DApp id (all-zero). Contracts deployed by an internal
/// message inherit their deployer's DApp — an order book deployed by a
/// PrivateNote (whose lineage starts at RootPN) lives in this DApp, not under
/// its own id.
///
/// This is a **network-config value, not a protocol constant**: a scheduled
/// network update moves DEX notes/books to DApp id `4`. This crate does not bake
/// the id in — every read/write helper takes the DApp explicitly (or `None` to
/// fall back to this default). After the migration, callers should pass the new
/// id; no code change here is required. Provided only as today's convenient
/// default.
