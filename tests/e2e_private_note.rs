...
/// **v2** (`09f596d5`) via `deploy_multisig`, giver-funded, and assert it is
/// Active with the requested code hash — i.e. a note-deploy-ready funding wallet.
#[tokio::test]
#[ignore]
async fn live_deploy_multisig_v2_via_giver() {
    ...
    let addr = deploy_multisig(
        ...
        DeployMultisigParams {
            kind: MultisigKind::UpdateCustodianV2,
            deployer: &deployer,
            owner_pubkeys: &[],
            req_confirms: 1,
    ...
    .await
    .expect("deploy UpdateCustodianMultisigWallet_v2");
    ...
    assert_eq!(
        got,
        MultisigKind::UpdateCustodianV2.code_hash(),
        "deployed code hash must be v2 09f596d5"
    );
    eprintln!(
        "=== DEPLOY_MULTISIG v2 PASSED — {addr} Active, code_hash={got} (deployer pubkey {}) ===",
        deployer.public_hex()
    );
}
...
async fn live_full_private_note_mint_via_giver() {
    ...
    //    (09f596d5), deployed via the public `deploy_multisig` (issue #5). v2's
    ...
    .await
    .expect("deploy v2 funding wallet");
    eprintln!("  v2 funding wallet Active: {wallet_addr}");
