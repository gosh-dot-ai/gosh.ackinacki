...
use gosh_ackinacki::private_note::artifacts::{ROOT_PN_ABI_JSON, ROOT_PN_ADDRESS};
...
async fn live_mint_fails_fast_on_wrong_wallet_type() {
    let client = ChainClient::shellnet().unwrap();
    let wrong_type = Address::parse(ROOT_PN_ADDRESS).unwrap(); // Active, not an UpdateCustodian wallet
    let halo2_dir = std::path::PathBuf::from("target/pn_e2e_halo2");
