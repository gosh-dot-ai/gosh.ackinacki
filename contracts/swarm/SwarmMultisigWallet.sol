...
contract SwarmMultisigWallet {
    ...
    uint64 constant MIN_BALANCE = 1 vmshell;

    /// Ensure contract has gas by minting from DappConfig.
    function _ensureBalance() private pure {
    ...
    */
    ...
    constructor(uint256[] owners_pubkey, address[] owners_address, uint8 reqConfirms, uint8 reqConfirmsData, uint64 value) {
        ...
        // Accept both external deploy (pubkey check) and internal deploy from SwarmRoot (sender check via salt)
        if (msg.sender == address(0)) {
            // External message: verify pubkey
            require(msg.pubkey() == tvm.pubkey(), 100);
        } else {
            // Internal message: verify sender is the SwarmRoot embedded in code salt
            optional(TvmCell) optSalt = abi.codeSalt(tvm.code());
            require(optSalt.hasValue(), 100);
            (string ver, address root, uint64 idx) = abi.decode(optSalt.get(), (string, address, uint64));
            ver; idx; // suppress unused warnings
